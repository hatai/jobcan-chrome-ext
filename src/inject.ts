// inject.ts — ページコンテキスト (MAIN world) で実行される
// 役割:
...
(function () {
   ...
   */
  function interceptSaveButton(): void {
   ...
   */
  function addSaveAndNextButton(): void {
  ...
  document.addEventListener('jce-open-edit', ((e: CustomEvent<JceOpenEditDetail>) => {
   ...
   */
  function triggerSave(mode: string): void {
    ...
    document.dispatchEvent(
      new CustomEvent<JceSaveValidatedDetail>('jce-save-validated', {
        detail: { mode: mode as JceSaveValidatedDetail['mode'] },
      })
    );
  }
})();
